...
| `hostus.base_url` | `SITUS_HOSTUS_BASE_URL` | `http://localhost:8080` | Basis-URL des hostus-Namensauflösers (`POST /v1/match`); beim `ingest` der `species_roles.csv` und zur Laufzeit ausschließlich für `POST /v1/species/habitat-types` (verbatim Namen) |
...
| `hostus.entry_backbone` | `SITUS_HOSTUS_ENTRY_BACKBONE` | `wcvp` | Taxonomisches Backbone, gegen das hostus matcht. Die gepinnten ESy-Namen sind Vaskularpflanzen, was `wcvp` abdeckt; eine hostus-Instanz auf einem anderen Backbone ist damit konfigurierbar statt neu zu kompilieren |
