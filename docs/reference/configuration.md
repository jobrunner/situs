...
| `index.path` | `SITUS_INDEX_PATH` | `situs.sqlite` | Pfad des lokalen SQLite-Index, aus dem die Lese-API antwortet. `situs ingest` schreibt ohne `--db` in genau diese Datei, damit Ingest und `serve` nicht auf verschiedene Indizes zeigen. `serve` öffnet **read-only**: ein fehlender Index bricht den Start ab (statt einen leeren anzulegen), ebenso eine Datei ohne `habitat_type`-Tabelle (leere Datei, abgebrochene Kopie, fremde Datenbank). Sonderzeichen im Pfad (`?`, `#`, `%`) werden escapt. Siehe `../how-to/deploy.md` |
