...
Gezählt wird nach `verbatim_name`, denn so ist die Tabelle geschlüsselt —
`(typology_id, code, verbatim_name, role)`. Es sind also (Habitattyp,
**Name**)-Paare, nicht (Habitattyp, Konzept)-Paare; die konzeptbezogene
Zählung steht darunter.

| Größe | Gemessen |
|---|---|
| Zeilen in `species_role` | **14696** |
| eindeutige (Habitattyp, **Name**)-Paare | **10470** |
...
| Zeilen ohne `concept_id` (nicht auflösbar) | 2173 |
...
#### Konzeptbezogen gezählt — und die zwei echten Duplikate

Die Leseroute fragt über die **Konzept-ID**, nicht über den Namen. Löst der
Ingest zwei `verbatim_name` auf dasselbe Konzept auf, erscheint derselbe
Habitattyp zweimal mit derselben Rolle. Gemessen:

```sql
SELECT COUNT(*) FROM (
  SELECT typology_id, code, concept_id, role
  FROM species_role WHERE concept_id IS NOT NULL AND concept_id <> ''
  GROUP BY 1,2,3,4 HAVING COUNT(DISTINCT verbatim_name) > 1);
-- 2
```
...
für `wcvp:concept:2623542` (*Aeonium aureum* / *Aeonium diplocyclum*,
`diagnostic`, Fidelity 17,9 und 16,9). Nur diese beiden sind echte Duplikate
im Sinne des Aufrufers; die Mehrfachnennung im Übrigen kommt von der Rolle.

#### Aggregat-Ableitungen tragen keine Kennzahl

```sql
SELECT provenance, COUNT(*),
       SUM(fidelity IS NOT NULL), SUM(constancy IS NOT NULL)
FROM species_role GROUP BY provenance;
-- derived_from_aggregate | 905   | 0    | 0
-- observed               | 13791 | 4377 | 9414
```

Alle 905 abgeleiteten Zeilen haben weder `fidelity` noch `constancy`: die
Kennzahl gilt dem Aggregat, nicht jeder Mitgliedsart darin.
