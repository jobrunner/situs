...
# table). CC-BY-4.0 — any redistribution of derived data must retain
# attribution to Dengler et al. (2023), Vegetation Classification and
# Survey 4: 7-29, https://doi.org/10.3897/VCS.98324.
#
# Emits pipe-delimited canonical CSV: taxon|vocab|vocab_version|dim|value|niche_width|n_systems
#
# convert.py imports openpyxl — a deliberate, documented exception to the
# stdlib-only pipeline convention (see README.md and CLAUDE.md's "Why XLSX
# parsing is not in the binary"), carried over unchanged from hostus.
set -euo pipefail

python3 -c "import openpyxl" 2>/dev/null || {
  echo "Fehlt: openpyxl (pip install openpyxl) — siehe README.md" >&2
  exit 1
}
